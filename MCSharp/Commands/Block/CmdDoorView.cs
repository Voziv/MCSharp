...
namespace MCSharp
{
    public class CmdDoorView : Command
    {
        public byte[] ignoreList = { 255, 255, 255, 255, 255, 255, 255, 0 }; // ignore air
        public byte type = (byte)0;
        public Block opMats = new Block();

        // Constructor
        public CmdDoorView(CommandGroup g, GroupEnum group, string name) : base(g, group, name) { blnConsoleSupported = false; /* By default no console support*/ }

        // Command usage help
        public override void Help(Player p)
        {
            p.SendMessage("/doorview - Shows all door_material as green blocks");
