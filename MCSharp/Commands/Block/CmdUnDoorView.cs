...
namespace MCSharp
{
    public class CmdUnDoorView : Command
    {
        public byte[] ignoreList = { 255, 255, 255, 255, 255, 255, 255, 0 };
        public byte type = (byte)0;
        public Block opMats = new Block();

        // Constructor
        public CmdUnDoorView(CommandGroup g, GroupEnum group, string name) : base(g, group, name) { blnConsoleSupported = false; /* By default no console support*/ }

        // Command usage help
        public override void Help(Player p)
        {
            p.SendMessage("/doorview - Reverts green showing door_material to original texture");
            p.SendMessage("/doorview <block> ignores that block, max 7");
        }
            ...
            if (message != "")
            {
                ...
                for (int i = 0; i < opMats.doorBlocks.Length; i++)
                {
                    if ((message.Contains(" " + Block.Name(opMats.doorBlocks[i]))) || (message.Contains(" " + Block.Name(opMats.regBlocks[i]))))
                    {
                        ...
                        counter += 1;
                        if (counter > ignoreList.Length - 2)
                        {
                            break;
                        }
                    }
