...
namespace MCSharp
{
    public class CmdDoor : Command
    {
        ...
        // Constructor
        public CmdDoor(CommandGroup g, GroupEnum group, string name) : base(g, group, name) { blnConsoleSupported = false; /* By default no console support*/ }

        // Command usage help
        public override void Help(Player p)
        {
            p.SendMessage("/door - Convert all blocks in an area to door_material");
            p.SendMessage("/door <block(s)>  ignores that block, max 7");
        }
