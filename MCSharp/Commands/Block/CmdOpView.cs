...
namespace MCSharp
{
    public class CmdOpView : Command
    {
        ...
        // Constructor
        public CmdOpView(CommandGroup g, GroupEnum group, string name) : base(g, group, name) { blnConsoleSupported = false; /* By default no console support*/ }
        
        // Command usage help
        public override void Help(Player p)
        {
            p.SendMessage("/opview - Shows all op_material as pink blocks");
            p.SendMessage("/opview <block> ignores that block, max 7");
        }
        ...
        public void Blockchange2(Player p, ushort x, ushort y, ushort z, byte type)
        {
            ...
            buffer.ForEach(delegate(Pos pos)
            {
                ...
                {
                    if (opMats.opBlocks.Contains(bl)) // if the block is an op_material
                    {
                        p.SendBlockchange(pos.x, pos.y, pos.z, ((byte)Block.pink)); // turn it pink
                    }
                }
            }
        );
            p.SendMessage("Opview complete.");
        }
